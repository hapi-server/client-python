...
PACKAGENAME = metadata.get('package_name', 'hapi_client')
DESCRIPTION = metadata.get('description', 'A Python client for the Heliophysics Application Programmer’s Interface')
AUTHOR = metadata.get('author', 'of services to enable access to data and metadata.Robert Weigel')
AUTHOR_EMAIL = metadata.get('author_email', '')
LICENSE = metadata.get('license', 'unknown')
URL = metadata.get('url', 'https://github.com/hapi-server/client-python')
__minimum_python_version__ = metadata.get("minimum_python_version", "3.5")

# Enforce Python version check - this is the same check as in __init__.py but
# this one has to happen before importing ah_bootstrap.
if sys.version_info < tuple((int(val) for val in __minimum_python_version__.split('.'))):
    sys.stderr.write("ERROR: hapi_client requires Python {} or later\n".format(__minimum_python_version__))
    sys.exit(1)
